<div class="region region-<?= $data['regionName']; ?> <?= ('admin' == \core\classes\SysAuth::getCurrentRole()) ? 'region-hover' : ''; ?>">

	<?php if ('admin' == \core\classes\SysAuth::getCurrentRole()): ?>
		<div class="setting-region">
			<div class="glyphicon glyphicon-cog" onclick="enableRegionTool(this)"></div>
		</div>
	<?php endif; ?>
	...
	<div class="region-content">
		<?php foreach ($data['items'] as $type => $blocks): ?>
			<?php foreach ($blocks as $block): ?>
				<div class="block block-<?= $type; ?> block-<?= $type; ?>-<?= $block['id']; ?> thumbnail"
					 id="block-<?= $type; ?>-<?= $block['id']; ?>"
					 data-type="<?= $type; ?>" data-id="<?= $block['id']; ?>">
					<?php if ('admin' == \core\classes\SysAuth::getCurrentRole()): ?>
